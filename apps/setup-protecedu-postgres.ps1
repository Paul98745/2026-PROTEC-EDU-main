...
try {
    ...
    Write-Step "Creando apps/backend/.env local"
...
"@ | Set-Content -LiteralPath $envPath -Encoding UTF8
    ...
    Write-Host ".env creado en apps/backend/.env (debe permanecer ignorado por Git)."
    ...
    try {
        npm --prefix apps/backend run db:targets:check
