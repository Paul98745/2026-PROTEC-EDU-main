...
- El archivo privado `apps/backend/.env` con la conexión local.

Ejecuta este comando desde la carpeta principal:

```powershell
pwsh -NoProfile -ExecutionPolicy Bypass -File .\apps\setup-protecedu-postgres.ps1 -RepoRoot (Get-Location).Path -PostgresBin "C:\Program Files\PostgreSQL\18\bin" -HostName 127.0.0.1 -Port 5433
```
...
Comprueba que exista `apps/backend/.env`, que PostgreSQL esté encendido y que la contraseña de `protecedu_app` sea correcta. Puedes volver a ejecutar el asistente del paso 5 para regenerar la configuración local.
...
Nunca subas `apps/backend/.env`, `node_modules`, `dist` ni contraseñas. El archivo `.gitignore` ya los excluye.
